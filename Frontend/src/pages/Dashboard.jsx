      ...
      <div className="relative z-10 max-w-6xl mx-auto space-y-8">
        ...
        >
          ...
          <div className="flex items-center gap-5 relative z-10">
            ...
            <div>
              ...
              <h1 className="font-outfit font-bold text-3xl text-gradient">Alex Johnson</h1>
              <div className="flex items-center gap-2 mt-2">
                <Badge variant="purple">CSE — 3rd Year</Badge>
                <Badge variant="cyan">Roll: 21CS047</Badge>
              </div>
            </div>
          </div>
          ...
          {stats.map(({ icon: Icon, label, value, color }) => (
            <motion.div key={label} variants={item}>
              <div className="glass glow-border-purple rounded-3xl p-5 flex items-center gap-4 group hover:bg-white/5 transition-all duration-300">
                <div className={`w-12 h-12 rounded-2xl bg-gradient-to-br ${color === 'text-primary-400' ? 'from-violet-600/30 to-purple-500/20' : color === 'text-pink-400' ? 'from-pink-500/30 to-rose-500/20' : color === 'text-cyan-400' ? 'from-cyan-500/30 to-teal-500/20' : 'from-emerald-500/30 to-teal-500/20'} flex items-center justify-center group-hover:scale-110 transition-transform`}>
        ...
        >
          {features.map(({ to, icon: Icon, label, desc, color, glow, badge, bVariant, delay }) => (
            <motion.div key={to} variants={item} custom={delay}>
              <TiltCard className={`glass glow-border-${glow} rounded-4xl p-6 cursor-pointer group`}
                onClick={() => navigate(to)}>
                ...
                <div className="mt-4 flex items-center gap-2 text-primary-400 text-sm font-outfit font-medium opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                  Open <span>→</span>
                </div>
              </TiltCard>
            </motion.div>
        ...
        <GlassCard glow="none" className="space-y-4">
          <div className="flex items-center gap-3 mb-2">
            ...
            <h2 className="font-outfit font-bold text-lg text-[var(--color-text)]">Announcements</h2>
          </div>
          {[
            { title: 'Mid-term exams schedule released', time: '2h ago', tag: 'Academic', color: 'purple' },
            { title: 'Cultural fest registrations open!',  time: '5h ago', tag: 'Event',    color: 'pink'   },
            ...
            >
              <div className={`w-2 h-2 rounded-full bg-gradient-to-br ${a.color === 'purple' ? 'from-violet-500 to-purple-400' : a.color === 'pink' ? 'from-pink-500 to-rose-400' : 'from-cyan-500 to-teal-400'} shrink-0`} />
